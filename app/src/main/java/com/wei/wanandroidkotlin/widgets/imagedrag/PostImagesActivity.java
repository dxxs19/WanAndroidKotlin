...
import com.wei.wanandroidkotlin.widgets.common.ViewWrapper;
...
public class PostImagesActivity extends AppCompatActivity {
    ...
    public static final String FILE_DIR_NAME = "imagesdrag";//应用缓存地址
    ...
    private void initData() {
        ...
        //添加按钮图片资源
        String plusPath = getString(R.string.glide_plus_icon_string) + AppUtils.getPackageInfo(mContext).packageName + "/mipmap/" + R.drawable.mine_btn_plus;
        ...
        rcvImg.addOnItemTouchListener(new OnRecyclerItemClickListener(rcvImg) {
            ...
            public void onItemClick(RecyclerView.ViewHolder vh) {
                if (originImages.get(vh.getAdapterPosition()).contains(getString(R.string.glide_plus_icon_string))) {//打开相册
                    ...
                    ToastUtils.getInstance().show(App.Companion.getAppContext(), "预览图片");
                }
            }
...
//                refreshLayout();
    ...
    private void largen(RecyclerView.ViewHolder vh) {
        View view = vh.itemView;
        ImageView imageView = view.findViewById(R.id.sdv);

        viewWrapper = new ViewWrapper(imageView);
    ...
    private void recover() {
        ...
        animatorSet.playTogether(
                // 同时缩小宽和高
                ObjectAnimator.ofInt(viewWrapper, "width", CommonUtils.getPixelById(R.dimen.article_img_dimens)),
                ObjectAnimator.ofInt(viewWrapper, "height", CommonUtils.getPixelById(R.dimen.article_img_dimens))
        );
    ...
    static class MyRunnable implements Runnable {
        ...
        public void run() {
            ...
            for (int i = 0; i < images.size(); i++) {
                if (images.get(i).contains(App.Companion.getAppContext().getString(R.string.glide_plus_icon_string))) {//说明是添加图片按钮
                    continue;
                }
                //压缩
                newBitmap = ImageUtils.compressScaleByWH(images.get(i),
                        DensityUtils.dp2px(App.Companion.getAppContext(), 100),
                        DensityUtils.dp2px(App.Companion.getAppContext(), 100));
                //文件地址
